...
class LoopbackCallbackServer:
    ...
    Supports multiple concurrent OAuth flows via a FIFO queue of futures.
    Each flow registers its expected state; the callback handler matches
    incoming callbacks to the correct future.
    """
    ...
    def __init__(self) -> None:
        ...
        self.redirect_uri = ""
        # For concurrent flow support (used by _reauthorize_async).
    ...
    def set_expected(self, state: str, iss: str | None = None) -> None:
        """Set the expected state and iss for CSRF validation.

        Must be called before the browser flow starts. The state is
        cryptographically generated by the caller (``secrets.token_urlsafe``).
        """
    ...
    async def _handle(
        ...
        try:
            ...
            if not target.startswith("/oauth/callback"):
                body = "<!doctype html><html><body><h2>404</h2></body></html>"
                response = (
            ...
            # Validate state (CSRF protection per RFC 6749 Section 10.12)
            if self._expected_state is not None:
                if state is None or not secrets.compare_digest(state, self._expected_state):
                    logger.warning("OAuth state mismatch: expected=%r received=%r", self._expected_state, state)
                    body = "<!doctype html><html><body><h2>Invalid state parameter</h2></body></html>"
                    response = (
            ...
            if self._expected_iss is not None and iss != self._expected_iss:
                logger.warning("OAuth iss mismatch: expected=%r received=%r", self._expected_iss, iss)
                body = "<!doctype html><html><body><h2>Invalid iss parameter</h2></body></html>"
                response = (
                    "HTTP/1.1 400 Bad Request\r\n"
                    "Content-Type: text/html; charset=utf-8\r\n"
                    f"Content-Length: {len(body.encode('utf-8'))}\r\n"
