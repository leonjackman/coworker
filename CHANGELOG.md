...
本版本让应用真正「打开即用」：窗口出现的那一刻，界面就已经是你所选语言的完整版本，不再需要先等后台就绪才“补上”翻译；同时把后端的就绪等待从 30 秒放宽到 1 分钟，慢电脑、冷启动或首次运行时不再被误判为故障。

### 新增

- **打开即本地化**：启动画面和主界面在窗口出现的瞬间即为完整本地化文本（支持全部 11 种语言），不再闪现未翻译的占位文字；后端仍在后台并行启动，无需前台等待。
- **启动更稳健**：将后端“就绪判定”的等待时间由 30 秒放宽至 1 分钟，低配设备 / 冷启动 / 首次安装（解压、安全扫描等）场景下不再因启动偏慢而被误判失败、提前退出。

### 改进

- **代码库全面模块化（功能与界面不变）**：将后端与前端的大单体文件按业务领域拆分为清晰的小模块，让后续迭代更快、更稳：
  - 后端按「会话 / 对话 / 记忆 / 设置 / 项目 / 技能 / 审批 / 提供商 / 终端」等能力拆分组织，共享状态统一管理，行为与上一版本完全一致。
  - 前端样式与聊天逻辑同样拆分为分领域模块，结构更清晰、更易维护。

### 修复

- 修复「重新生成回复」时偶发的流式输出中断错误，让重跑会话更稳定。
- 清理动画等冗余定义，减少维护成本、提升代码整洁度。

### 品质

- 通过后端全量自动化测试与逐端点比对，确认重构前后功能一致；前端类型检查与构建全部通过，并在真实环境中完成启动与界面验证。

## 0.6.7

本版本聚焦「上下文剪裁不再讓你失憶」：記憶注入更準確完整、上下文被收緊時 Agent 永遠知道「缺了什麼、去哪補」，並讓「閱讀另一個會話」變成可完整翻頁讀取，而不是只給到一半。

### 新增

- **記憶注入更完整（記憶更準確）**：記憶索引現在顯示每個檔案的區塊數與行數（`blocks`/`lines`）；多行區塊不只給首行，會展開最多前 3 行並標註「尚有 N 行，可用 memory_read 讀取完整內容」。Agent 明確知道哪些記憶只是預覽、會主動取回全文——長時記憶不再「看起來只有一句話」。
- **跨會話閱讀改為分頁（可讀完整）**：當你貼上另一個專案的會話 id、叫 Agent 閱讀時，`read_session` 現在從「最近的訊息」開始分頁返回，附 `next_offset` 提示；Agent 會自動向歷史翻頁直到 `next_offset=0`，**整段會話（含最早決策）都能讀完**。舊版只給最早約 6 萬字元、最新決策反而看不到，且無法續讀。

### 改動

- **剪裁改為「明示省略 + 保留指針」**：上下文緊張時收緊舊工具結果，從「只留開頭」改為「欄位級頭尾保留」——`read_file` 的分頁位置（`next_offset`）與 `run_command` 的完整輸出磁碟路徑仍原樣保留，續頁與取回不再因剪裁而斷。
- **清除不再是死路**：舊工具結果被清掉時，佔位提示會告訴模型這是哪個工具的輸出、以及如何取回（命令輸出指出落盤路徑；讀檔提示用相同參數重新讀取），而非一個無資訊的 `[cleared]`。
- **系統提示詞被壓縮時會告知模型**：若固定提示詞預算超限需要省略技能目錄 / 記憶索引等片段，會向模型明確說明「本次未注入哪些、可用 load_skill / memory_read 取回」，不再靜默刪除。
- **緊急壓縮先清佔位、後丟對話**：摘要模型不可用時的緊急修剪，會先清掉佔空間的舊命令輸出，而不是優先刪除對話中的決策與你的最近輸入。
- **計數口徑統一**：各層工具清除的觸發計數統一改用 CJK 感知估算，長中文會話的觸發點更精準。

### 修復

- 修復記憶預覽遇到「大區塊」時會停止渲染後續檔案、讓同檔其餘記憶被餓死的問題。
- 統一工具結果剪裁與清除的入口，消除多處重複定義與舊佔位符殘留。
