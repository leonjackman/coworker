...
| 2026-08-12 | TodoBlock 宽度 | 修复 TodoBlock 超长文本溢出：实测 300 字符无空格 todo 把卡片撑到 2465px（槽宽 1292）水平溢出、文本不换行。约束 .todo-block 宽度为 min(var(--bottom-card-width),100%) 居中 + .goal-card__todo span 加 overflow-wrap/word-break/min-width:0 允许长词换行。实测修复后卡宽 1024px 与 composer 一致、无溢出、多行换行。 |
