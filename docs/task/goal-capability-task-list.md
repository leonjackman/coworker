...
- [ ] Step 1.4：Electron IPC 桥接层（打通前端 → 后端 `/goal`，已拍板纳入）
...
- [ ] Step 2.1：单一生成器内层多轮续跑循环（多轮包进传给 `_publish_turn` 的 `stream_iter`）
...
- [ ] Step 4.1：注册 `manage_goal` + `get_goal` 模型工具
  - [ ] 预期目标：模型在目标模式下可声明完成/受阻（`manage_goal`）、主动读取目标状态（`get_goal`）；无权改 paused/resume/budget。
  - [ ] 行为：仅当 session 有 active goal 时工具可见；`manage_goal` 调用后持久化状态，续跑循环据此停止（complete/blocked）；`get_goal` 只读查询。
  - [ ] 改动点：
    ...
    - 工具执行体：`manage_goal` 调用 `session_store.update_goal_status(session_id, status)` 并 `_emit_goal_updated(...)`；遇非 active 时报错提示「当前没有 active 目标」。`get_goal` 返回当前 `GoalState | None`（仅查询）。
...
- [ ] Step 5.2：TodoBlock 内 goal section（复用现有卡片与位置，不另设 GoalBar）
...
- [ ] Step 5.3：`/goal` 聊天命令 + TodoBlock 按钮
...
- [ ] Step 7.1：手动链路验证（替代失败测试，按用户豁免）
  - [ ] 预期目标：核心行为可端到端复现。
  - [ ] 行为：
    - `/goal 写一个计算器 CLI` → TodoBlock 显示 goal（active），**空闲会话下自动起跑**，自动连跑多轮直至 `manage_goal(complete)`。
