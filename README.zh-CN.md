...
| 🧭 **桌面操控 (OS Computer Use)** | 桌面级控制：截图 + 任意原生 App 中的全局点击 / 拖拽 / 滚动 / 输入。总开关默认关闭；默认权限下每次操作都需确认（⇧⌘⎋ 可暂停） |
...
Coworker 内置一个系统保留的「聊天」项目，固定在侧栏顶部，无需先创建项目即可直接开始闲聊。

- **开箱即聊** — 首启引导页提供「直接开始聊天」；当还没有任何真实项目时，全局「新对话」会自动落到聊天项目。
- **常驻且安全** — 启动时自动创建，文件夹被误删也会自愈，且不可删除、不可重命名。其工作区为系统指定的独立沙箱目录（`COWORKER_DATA_DIR/chat`），与真实项目完全隔离。
- **Lazzzy Boy 人设** — 聊天项目里的 Agent 采用来自 [Lazzzy Boy](https://lazzzyboy.com) 工作室的「懒懒男孩」人设：轻松、随和、聪明，回答简洁。除非你明确要求，否则它不会读取或修改文件、运行命令或调用搜索 / MCP 工具，只好好聊天。（工具仍然保留，你要求时它也能在沙箱里干活。）
- **名称本地化** — 项目名称跟随界面语言显示（聊天 / Chat / チャット …）。
...
| 无法控制 Agent 行为 | **默认 HITL** — 默认权限下自动放行工作区内动作，越界操作需手动确认 |
...
- **手动写入**：agent 通过 `memory` 工具把持久事实写入自己的
  `BASE/MEMORY.md`（或主题文件 / `SESSIONS/<日期>.md`），自动去重，默认权限
  下需审批。
...
每个项目都有一份只读仪表盘，让项目的文件、Agent、Git 状态与会话历史一目了然。

- **五个标签页** —— 概览（项目信息、Git 状态与统计）· 文件 · Agent · 记忆 · 会话历史。
- **文件浏览器** —— 支持键盘导航的文件树（ARIA 可访问），提供富预览：带行号的代码高亮、CSV / XLSX 虚拟表格滚动与可拖拽分栏。
- **外部打开** —— 可在仪表盘直接用系统默认应用打开任意文件。
- **快速访问** —— 从侧栏或 `Cmd+O` 快捷键打开。
