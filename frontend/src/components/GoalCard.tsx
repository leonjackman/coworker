import { Check, ChevronDown, Pause, Pencil, Play, Target, Trash2 } from 'lucide-react';
...
export function GoalCard({ goal, onPause, onResume, onDelete, onDraftEdit }: GoalCardProps) {
  ...
  return (
    <div className="goal-card" data-state={goal.done ? 'done' : goal.paused ? 'paused' : 'active'}>
      ...
      <div className="goal-card__body">
        ...
        {!goal.done && goal.progress && <p className="goal-card__progress">{goal.progress}</p>}
      </div>
    </div>
  );
}
