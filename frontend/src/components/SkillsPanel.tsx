...
import { Loader2, RefreshCw, Search, Shield, ShieldCheck, ShieldX, FileText, AlertTriangle } from 'lucide-react';
...
export function SkillsPanel({ skills, diagnostics, setSkills, setDiagnostics, onSkillsChange }: SkillsPanelProps) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setMessage(t('skills.failed_to_load'));
    } finally {
      setLoading(false);
    }
    ...
    <div className="workspace-page__content">
      <div className="mcp-list-heading">
        ...
        <div className="mcp-list-actions">
          ...
          <Button variant="ghost" onClick={rescan} disabled={loading} className="mcp-check-all-btn">
      ...
      {message && (
        <div className={`mcp-form-message ${messageType === 'error' ? 'mcp-form-message--error' : ''}`}>
          {message}
        </div>
      )}
      ...
      {diagnostics.length > 0 && (
        <div className="mcp-diagnostics">
          <div className="mcp-diagnostics__title">
            <AlertTriangle size={14} /> {t('skills.diagnostics')} ({diagnostics.length})
          </div>
          {diagnostics.map((diagnostic, index) => (
            <div key={index} className={`mcp-diagnostics__item mcp-diagnostics__item--${diagnostic.type}`}>
              <strong>{diagnostic.type}</strong> {diagnostic.name}
              <span className="mcp-diagnostics__message">— {diagnostic.message}</span>
            </div>
          ))}
        </div>
      )}
      ...
      {skills.length === 0 ? (
        ...
        <div className="mcp-list">
          {filtered.map((skill) => {
            ...
            return (
              <article
                className={`mcp-card ${!skill.enabled ? 'mcp-card--disabled' : ''}`}
                key={skill.name}
                onClick={() => handleExpand(skill)}
              >
                <div className="mcp-card__top">
                  <div className="mcp-card__identity">
                    <span className={`mcp-status-dot ${skill.enabled ? 'mcp-status-connected' : 'mcp-status-disabled'}`} />
                ...
                <div className="mcp-card__meta">
                  <span className="mcp-card__subtitle">{skill.description}</span>
                </div>

                {isExpanded && (
                  <div className="mcp-card__detail" onClick={(e) => e.stopPropagation()}>
                    {detailLoading ? (
      ...
      {/* Validate tool */}
      <div className="mcp-quick-card" style={{ marginTop: 16 }}>
        <FileText size={16} className="mcp-form-card__icon" />
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {validateResult && (
            <div className={`skill-validate-result ${validateResult.valid ? 'skill-validate-result--ok' : 'skill-validate-result--bad'}`}>
